            ...
            class MatchBattlesCtrl {
                ...
                private createPlayerEvents(item: any) {
                    if (item["EventName"] === "UnitTrained") {
                        if (this.cleanItem(item)) {//item["SquadId"].includes("cov") || item["SquadId"].includes("unsc")) {
                ...
                // Temporal Fix for Serina's Ice Block, Colony's Living Barrier and Johnson's Bunker.
                private cleanItem(item: any): boolean {
                    if (item["SquadId"].includes("bldg")) {
                        return false;
                    }
                ...
                selectUnit($event: any, unit: any) {
                    this.$mdBottomSheet.show({
                        ...
                        controller: ["$scope", "$mdBottomSheet", "Item", UnitPanelController],
                        locals: {
                            Item: {
                                "unit": unit
                            }
